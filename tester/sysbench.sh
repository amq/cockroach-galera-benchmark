...
export COCKROACHDB_SYSBENCH="sysbench --db-driver=pgsql --pgsql-host=$COCKROACHDB_HOST --pgsql-port=$COCKROACHDB_PORT --pgsql-user=$COCKROACHDB_USER --mysql-password=$COCKROACHDB_PASS"

export GALERA_HOST=galera-01
export GALERA_PORT=3306
export GALERA_USER=root
export GALERA_PASS=""
export GALERA_SYSBENCH="sysbench --db-driver=mysql --mysql-host=$GALERA_HOST --mysql-port=$GALERA_PORT --mysql-user=$GALERA_USER --pgsql-password=$GALERA_PASS"
...
for test in oltp_read_write oltp_read_only oltp_write_only oltp_update_non_index; do
    ...
    time $COCKROACHDB_SYSBENCH $test --tables=$TABLES --table-size=$TABLE_SIZE prepare

    for threads in 1 2 4 8 16 32 64 96 128 192 256; do
        sleep $WARMUP_TIME
        echo "cockroachdb: started sysbench $test ($threads threads) at $(date +'%H:%m')"
        time $COCKROACHDB_SYSBENCH $test --histogram=on --report-interval=10 --warmup-time=$WARMUP_TIME --rand-type=zipfian --time=60 --threads=$threads run
...
for test in oltp_read_write oltp_read_only oltp_write_only oltp_update_non_index; do
    ...
    time $GALERA_SYSBENCH $test --tables=$TABLES --table-size=$TABLE_SIZE prepare

    for threads in 1 2 4 8 16 32 64 96 128 192 256; do
        sleep $WARMUP_TIME
        echo "galera: started sysbench $test ($threads threads) at $(date +'%H:%m')"
        time $GALERA_SYSBENCH $test --histogram=on --report-interval=10 --warmup-time=$WARMUP_TIME --rand-type=zipfian --time=60 --threads=$threads run
